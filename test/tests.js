(function() {
  ...
  describe("gulp-jsclosure Test Suite:", function() {
    describe("Testing Null Options Param:", function() {
      ...
      // Setup stream
      beforeEach(function() {
        var rawFile = fs.readFileSync("./test/inputFiles/in.js", "utf8");
        testFile = new gutil.File({
          contents: new Buffer(rawFile)
        });
        var outFile = fs.readFileSync("./test/outputFiles/1.js", "utf8");
    ...
    describe("Testing Object type Options Param:", function() {
      ...
      it("Return file with closure (2 params) - closure({\"$\":\"jQuery\", document:true})", function (done) {
        var stream = closure({"$":"jQuery", document:true});
